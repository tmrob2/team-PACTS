...
def new_target(X, W, t, l, m, n, ii, cstep, pstep):
    ...
    z = {i: LpVariable(name=f"z{i}", upBound=upper_bound(t, ii, pstep, j, cstep)) for i in range(n + m)}
    ...
    model += lpSum(lambda_.values()) == 1

    model += epsilon

    status = model.solve(solver)
    solution = [z[j] for j in range(n + m)]
    return solution

def upper_bound(t, ii, prob_step, j, cstep):
    if j < n:
        t[j] - ii * cstep
    else:
        if t[j] - ii * prob_step:
