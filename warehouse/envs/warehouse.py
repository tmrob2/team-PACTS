...
class Warehouse(gym.Env):
    ...
    def label(self, q: int, agent: int, task_racks: List, task: int, feedpoints: List):
        """
        ...
        if q == 0:
            if all(np.equal(np.array(obs["a"]), np.array(task_racks[task]))):
                if obs["c"] == 0:
                    return "a"
                else:
                    return "fail"
            ...
            if obs["c"] == 1:
                if all(np.equal(np.array(feedpoints[task]), np.array(np.array(agent_rack_pos)))):
                    return "cf"
                else:
                    return "ncf"
            else:
            ...
            if obs["c"] == 1:
                if all(np.equal(np.array(task_racks[task]), np.array(agent_rack_pos))) \
                    and all(np.equal(np.array(obs["a"]), np.array(task_racks[task]))):
                    return "ca"
                else:
                    return "nca"
            else:
            ...
            raise ValueError(f"DFA state {q} not found")
